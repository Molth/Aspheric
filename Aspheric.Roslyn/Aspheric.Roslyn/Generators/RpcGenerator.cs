...
namespace Erinn.Roslyn
{
    ...
    internal sealed class RpcIncrementalSourceGenerator : IIncrementalGenerator
    {
        ...
        private static void GenerateAssemblyRpc(string assembly, StringBuilder sb, ServiceData serviceData, SourceProductionContext context, List<(string FullName, bool HasNamespace, ServiceTarget Target)> rpcClasses)
        {
            ...
            sb.AppendLine("\t{");
            sb.AppendLine($"\t\tpublic const uint RPC_METHOD_COUNT = {serviceData.RpcMethodCount};");
            sb.AppendLine($"\t\tpublic const uint ON_CONNECTED_COUNT = {serviceData.OnConnectedCount};");
            sb.AppendLine($"\t\tpublic const uint ON_DISCONNECTED_COUNT = {serviceData.OnDisconnectedCount};");
            sb.AppendLine($"\t\tpublic const uint ON_ERRORED_COUNT = {serviceData.OnErroredCount};");
            sb.AppendLine($"\t\tpublic const uint ON_RECEIVED_COUNT = {serviceData.OnReceivedCount};");
            sb.AppendLine();
